...
class DatabaseAdaptor {
	...
	public function resetPassword($user,$newPassword)
	{
		...
		$stmt = $this->DB->prepare("update users set hash = :hashed_pwd where username = :username");
		$stmt->bindParam( 'hashed_pwd', $hashed_pwd );
		$stmt->bindParam( 'username', $user );
		$stmt->execute ();
		return;
    ...
    {
        $stmt = $this->DB->prepare("update users set Bg =:bg where username =:username");
    ...
    public function recordLoginTime($user, $date)
    {
        $stmt = $this->DB->prepare("update users set LoginDate =:date where username =:username");
    ...
    public function recordLogoutTime($user, $date)
    {
        $stmt = $this->DB->prepare("update users set LogoutDate =:date where username =:username");
        $stmt->bindParam('date', $date);
        $stmt->bindParam('username', $user);
        $stmt->execute ();
        return;
    ...
    public function checkTodaySale($date)
    {
        $stmt = $this->DB->prepare("select id, users.username, orderhistory.item from users
                                    join orderhistory on users.username = orderhistory.username
                                    where date =:date");
        $stmt->bindParam('date', $date);
        $stmt->execute ();
        return $stmt->fetchAll ( PDO::FETCH_ASSOC );
