...
if(isset($_GET["USER"])&&isset($_GET["PASSWORD"]))
{
    ...
    if(count($arr)==0){
        ...
        if($arr[0]["Admin"] == 1)
            ...
            $_SESSION['Admin'] = 0;       
        //if succeed, record the login time
        date_default_timezone_set("America/Phoenix");
        $date = date ( "Y-m-d H:i:s");
        $theDBA->recordLoginTime($user, $date);
        header('Location: index.php');
    }
    else
    {
...
{
    $user =  $_SESSION['login'];
    //record the time user log out
    date_default_timezone_set("America/Phoenix");
    $date = date ( "Y-m-d H:i:s");
    $theDBA->recordLogoutTime($user, $date);
...
if(isset($_GET["review"]))
{
 ...
 {
     $oldPwd = htmlspecialchars($_POST["UserEnterPwd"]);
     $user = $_SESSION['login'];
     $arr = $theDBA->check($user);
     
     if(password_verify($oldPwd, $arr[0]['hash']))
     {
        $newPassword = htmlspecialchars($_POST["NewPwd"]);
 ...
 if(isset($_GET["display"]) && $_GET["display"] === "show Info")
 {
     $user=$_SESSION['login'];
     $arr=$theDBA->check($user);
     echo json_encode ( $arr );
 ...
 if(isset($_GET["getTodaySale"]))
 {
     date_default_timezone_set("America/Phoenix");
     $mydate = date ( "d-M-Y" );
     $arr = $theDBA->checkTodaySale($mydate);
     echo json_encode ( $arr );
 }
 
 //------------------------------------------------------------------------
 function decode_helper(& $array)
 {
     for($i=0; $i<sizeof($array); $i++)
     {
         $array[$i]['review'] = htmlspecialchars_decode($array[$i]['review']);
     }
 }
